...
# Added by Jaunie
def outer_ellipsoid_fit_2(points):
    """
    Outer ellipsoid using the minimal upper bound algorithm.
    Returns (A, center)
    """
    # For outer ellipsoid, we want bounding matrix from point vectors
    matrices = [np.outer(p, p) for p in points]
    M0 = sum(matrices)
    matrices.insert(0, M0)

    _, M = mf.minimality_check(matrices, use_rand=True)
